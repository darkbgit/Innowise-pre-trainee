...
public class AuthorsController(IAuthorService authorsService) : ControllerBase
{
    ...
    [HttpGet("with-books-number")]
    public async Task<ActionResult<IReadOnlyList<AuthorDto>>> GetWithBooksNumber()
    {
        var result = await _authorsService.GetAllAuthorsWithBooksNumberAsync();
        return Ok(result);
    }

    [HttpGet("search")]
    public async Task<ActionResult<IReadOnlyList<AuthorDto>>> SearchAuthors([FromQuery] string? query, [FromQuery] DateOnly? bornAfter)
    {
        var result = await _authorsService.SearchAuthorsAsync(query, bornAfter);
        return Ok(result);
    }
